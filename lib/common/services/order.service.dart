...
Future<CurrentOrderModel> createNewOrder(CreateOrderDtoModel order) async {
  try {
    final Response<dynamic> res =
        await api.dio.post('/orders/create', data: order);
...
Future<List<CurrentOrderModel>> getOrdersLog({int page = 1}) async {
  try {
    final Response<dynamic> res = await api.dio.get('/orders?page=$page');
...
Future<CurrentOrderModel> getCurrentOrder(String orderId) async {
  try {
    final Response<dynamic> res =
        await api.dio.get('/orders/get-order/$orderId');
